...
def main(cells_list, prep,  rack, path, sources, fictive, volume, chaser, delay, verbose, lap, ID, prompts, chem, prep_container):
    if lap == 1:
        ld, c = LS_sample(cells_list, prep,  rack, path, sources, fictive, volume, chaser, delay, verbose)
    else:
        ...
        ld._prompts = prompts
        ld._chem = chem
        c = [ld.timestamp_container(prep_container)]
        ld.save_container(c[0])
    return ld, c
...
def LS_sample(cells_list, prep,  rack, path, sources, fictive, volume, chaser, delay, verbose):
